 ...
 * @version    v1.0
 */
class Validate_Integer extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は整数で入力してください。",
    );
    ...
    /**
     * セットされたパラメータが整数で入力されているかチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
        if(strval($this->getParam())==strval(intval($this->getParam()))) {
            return true;
        } else {
