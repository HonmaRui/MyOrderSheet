 ...
 * @category   ECSite
 * @package    Validate
 * @subpackage MaxLengthEm
 * @copyright  Copyright (c) 2014 Majestic-PIC Co., Ltd.
 * @author     鈴木
 * @version    v1.0
 */
class Validate_MaxLengthEm extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は全角%Threshold%文字以下で入力してください。",
    );
    ...
    /**
     * セットされたパラメータが閾値で設定された文字長以下かチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
        //$iLength = mb_strwidth($this->getParam());
        $iLength = CommonTools::get_string_length($this->getParam());
        
        if($iLength <= $this->getThreshold() * 2) {
            return true;
        } else {
