 ...
 * @category   ECSite
 * @package    Validate
 * @subpackage AlphaNumericSpace
 * @copyright  Copyright (c) 2014 Majestic-PIC Co., Ltd.
 * @author     鈴木
 * @version    v1.0
 */
class Validate_AlphaNumericSpace extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は半角英数字・半角スペースで入力して下さい。",
    );
    ...
    /**
     * セットされたパラメータが英数字のみで構成されているかチェックする。
     *
     * @access public
     * @return mixed
     */
...
//        if(ctype_alpha($this->getParam())) {
        if(preg_match("/^[a-zA-Z0-9\s]+$/", $this->getParam())) {
            return true;
        } else {
