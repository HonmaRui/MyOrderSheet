 ...
 * @category   ECSite
 * @package    Validate
 * @subpackage MetaWithHyphen
 * @copyright  Copyright (c) 2014 Majestic-PIC Co., Ltd.
 * @author     鈴木
 * @version    v1.0
 */
class Validate_MetaWithHyphen extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%にハイフン以外の記号が含まれています。記号を削除してから再度送信して下さい。",
    );
    ...
    /**
     * セットされたパラメータにハイフンを除くメタ文字が含まれているかチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
        $stParam = str_replace("-", "", $this->getParam());
        $stParam = str_replace("ー", "", $stParam);
        $stParam = str_replace(array(" ", "　"), "", $stParam);
