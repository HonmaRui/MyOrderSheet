 ...
 * @category   ECSite
 * @package    Validate
 * @subpackage CellPhone
 * @copyright  Copyright (c) 2014 Majestic-PIC Co., Ltd.
 * @author     鈴木
 * @version    v1.0
 */
class Validate_CellPhone extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は000-0000-0000の形式で入力してください。",
    );
    ...
    /**
     * セットされたパラメータが携帯電話番号(ハイフン有り)の形式で入力されているかチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
        if(preg_match( "/^0[1-9]0\-?\d{4}\-?\d{4}$/" , $this->getParam())) {
            return true;
        } else {
