 ...
 * @version    v1.0
 */
class Validate_ZipCode extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は000-0000の形式で入力してください。",
    );
    ...
    /**
     * セットされたパラメータが郵便番号(ハイフン有り)の形式で入力されているかチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
        if(preg_match('/^\d{3}\-?\d{4}$/', $this->getParam())) {
            return true;
        } else {
