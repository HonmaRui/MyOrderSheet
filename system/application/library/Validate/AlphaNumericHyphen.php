 ...
 * @category   ECSite
 * @package    Validate
 * @subpackage AlphaNumericHyphen
 * @copyright  Copyright (c) 2014 Majestic-PIC Co., Ltd.
 * @author     鈴木
 * @version    v1.0
 */
class Validate_AlphaNumericHyphen extends Validate_Abstract {
    /**
     * エラーメッセージ用文章
     * @var array
     */
    protected $_arrMessageTemplates = array(
        "Invalid" => "%ColumnName%は半角英数字ハイフン(-)で入力して下さい。",
    );
    ...
    /**
     * セットされたパラメータが英数字 + ハイフンのみで構成されているかチェックする。
     *
     * @access public
     * @return mixed
     */
    public function isValid() {
//        if(ctype_alpha($this->getParam())) {
//        if(preg_match('/\A\w[\w+\- ]+$/', $this->getParam())) {
        if(preg_match('/^[a-zA-Z0-9\-]+$/', $this->getParam())) {
            return true;
        } else {
