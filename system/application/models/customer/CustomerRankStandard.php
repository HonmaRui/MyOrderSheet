 ...
 * @version    v1.0
 */
class Application_Model_CustomerRankStandard extends Application_Model_Abstract {

    /**
     * テーブル名
     * @var
     */
    protected $_stTableName = "d_customer_rank_standard";
    ...
    public function save($arrData) {
        ...
        try {
            ...
            $arrParams = $arrData;
            $stWhere = $this->objMasterDb->quoteInto("d_customer_rank_standard_CustomerRankStandardID = ? ", $arrData["d_customer_rank_standard_CustomerRankStandardID"]);
...
//            $this->objMasterDb->update($stTable, $arrParams, $arrWhere);
     ...
     * 第一引数による逆LIKE検索
     * 
     * @param   string   $stKeyword          キーワード
     * @return  array    $arrResult          検索結果
     */
    public function findRank($stKeyword) {
        
        try {
            $arrResult = array();

            // select文を実行する
            $objSelect = "SELECT * FROM `d_customer_rank_standard` WHERE '". $stKeyword . "' LIKE concat('%',`d_customer_rank_standard_CustomerRankStandardName`,'%') ";
            $objSql = $this->objSlaveDb->query($objSelect);
            // 検索結果を $arrResultに格納
            $arrResult = $objSql->fetchAll();
