 ...
 * @version    v1.0
 */
class Application_Model_CustomerRank extends Application_Model_Abstract {

    /**
     * テーブル名
     * @var
     */
    protected $_stTableName = "d_customer_rank";
    ...
    public function save($arrData) {
        ...
        try {
            ...
            $arrParams = $arrData;
            $arrParams["d_customer_rank_UpdatedTime"] = date("Y-m-d H:i:s");
            $arrParams["d_customer_rank_UpdatedByID"] = $this->objAdminSess->MemberID;
            $stWhere = $this->objMasterDb->quoteInto("d_customer_rank_CustomerRankID = ? ", $arrData["d_customer_rank_CustomerRankID"]);
    ...
    public function find($arrWhere, $arrColumn = "") {
        ...
        try {
            ...
            $objSelect->where("d_customer_rank_DelFlg = ? ", 0);
  ...
  /**
    ...
    public function fetchAll($arrColumn = "", $arrOrder = "") {
        ...
        try {
            ...
            $objSelect = &$this->objSlaveDb->select()->from(array("cr" => $this->getTableName()), $arrColumn);
     ...
     * 第二引数が指定されている場合、その配列を取得対象カラムとする。
     * 
     * @param   string  $stMinColumn  最大値を取得するカラム
     * @param   array   $arrColumn    取得対象のカラム名を格納した配列
     * @return  array   $arrResult    検索結果
     */
    public function min($stMinColumn, $arrColumn = "") {
        ...
        try {
            ...
            $objSelect = &$this->objSlaveDb->select()->from(
                array($this->getTableName()), array(new Zend_Db_Expr("min(" . $stMinColumn . ")")));
            $objSelect->where("d_customer_rank_DelFlg = ? ", 0);
    ...
    public function insert($arrData) {
        ...
        try {
            ...
            $stTable = $this->getTableName();
            $arrParams = $arrData;
            $arrParams["d_customer_rank_CreatedTime"] = date("Y-m-d H:i:s");
            $arrParams["d_customer_rank_UpdatedTime"] = date("Y-m-d H:i:s");
            $arrParams["d_customer_rank_CreatedByID"] = $this->objAdminSess->MemberID;
            $arrParams["d_customer_rank_UpdatedByID"] = $this->objAdminSess->MemberID;
            $arrParams["d_customer_rank_DelFlg"] = "0";
    ...
    public function delete($iID) {
        ...
        try {
            ...
            $stTable = $this->getTableName();
            // 更新内容
            $arrParams["d_customer_rank_DelFlg"] = "1";
            $arrParams["d_customer_rank_UpdatedTime"] = date("Y-m-d H:i:s");
            $arrParams["d_customer_rank_UpdatedByID"] = $this->objAdminSess->MemberID;
            // 更新条件
            $arrWhere["d_customer_rank_CustomerRankID = ?"] = $iID;
            $arrWhere["d_customer_rank_DelFlg = ?"]  = "0";
